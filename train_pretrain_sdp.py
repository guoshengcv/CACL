...
from datasets.ucf101 import UCF101SDPDataset
...
def train(args, model, criterion, optimizer, device, train_dataloader, writer, epoch, lr):
    ...
    for i, data in tqdm(enumerate(train_dataloader, 1)):
        # get inputs
        stacked_clip, tuple_orders = data
        stacked_clip = stacked_clip.to(device)
        ...
        h_outputs = model(stacked_clip) # return logits here
        ...
        if i % args.pf == 0:
            ...
            print('[TRAIN] epoch-{}, batch-{}, clsloss: {:.3f}, clsacc: {:.3f}, lr:{:.6f},'.format(epoch, i, reduced_avg_loss.item(),
                     reduced_avg_acc.item(),lr))
...
def validate(args, model, criterion, device, val_dataloader, writer, epoch):
    ...
    reduced_avg_loss = reduce_mean(avg_loss, args.ngpus_per_node)
    reduced_avg_acc = reduce_mean(avg_acc, args.ngpus_per_node)
    print('[VAL] epoch-{}, batch-{}, clsloss: {:.3f}, clsacc: {:.3f}'.format(epoch, i, reduced_avg_loss.item(),reduced_avg_acc.item()))
...
def test(args, model, criterion, device, test_dataloader):
    ...
    for i, data in tqdm(enumerate(test_dataloader)):
        ...
        # zero the parameter gradient
        # forward and backward
        h_outputs = model(stacked_clip_q, stacked_clip_k)  # return logits here
        pts = torch.argmax(h_outputs, dim=1)
        correct += torch.sum(targets == pts)
        correct = correct.type(torch.float)
    avg_acc = correct / (i + 1) / args.bs
    reduced_avg_acc = reduce_mean(avg_acc, args.ngpus_per_node)
    print(f"test acc {reduced_avg_acc}")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    cnn_base = Wrapper_Model(base_network=base, feature_size=512, tuple_len=args.cl, with_contrast=False)
    ...
    if args.mode == 'train':  ########### Train #############
        ...
        train_dataset = UCF101SDPDataset('data/ucf101', args.cl, args.it, True, moco_transforms, uniform_sdp=False)
        ...
        for epoch in range(args.start_epoch, args.start_epoch+args.epochs):
            ...
            train(args, model, criterion, optimizer, args.gpu, train_dataloader, writer, epoch,lr)
